...
function StatusIcon({ status }: { status: SubscriptionRead["status"] }) {
  ...
  if (status === "canceled") return <XCircle className="h-5 w-5 text-muted-foreground" />;
      ...
      <CardContent className="space-y-4">
        ...
        {subscription.cancel_at_period_end && (
          <Alert>
            ...
            <AlertDescription>
              Your subscription will cancel on <strong>{periodEnd}</strong>. You&apos;ll retain access until then.
            </AlertDescription>
          </Alert>
        )}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Your subscription will remain active until <strong>{periodEnd}</strong>, then cancel automatically.
